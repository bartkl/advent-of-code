...
class HeightMap:
    ...
    async def _fill_basin_on_map(self, basin: Set[Coords], hmap, basin_tag):
        ...
        return await self._fill_basin_on_map(relevant_neighbours, hmap, basin_tag)

    async def fill_basins_on_map(self) -> List[List[Any]]:
        ...
        fill_tasks_per_low_point = [self._fill_basin_on_map({p}, hmap_data, next(basin_tag_generator)) for p in low_points]
        await asyncio.gather(*fill_tasks_per_low_point)
...
async def main():
    ...
    # Answer 2.
    basins = await height_map.fill_basins_on_map()
